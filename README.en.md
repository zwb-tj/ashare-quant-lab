...
**A from-scratch, reproducible A-share research platform**: explicit execution costs, lookahead-free
signal execution, falsifiable statistical claims, and an **auditable tool-using LLM research agent**
behind a testable CLI.

![equity curves](docs/assets/equity_curves.png)

> Not another "doubling strategy" repository. This one writes **research discipline** into code: costs
> must be explicit, execution must be delayed by one bar, conclusions must be falsifiable, and **every
> number an agent states must come from a tool**. **Negative results are kept** - several hypotheses were
> rejected by the project's own data.
...
- [Measured output (reproducible, synthetic bars)](#measured-output-reproducible-synthetic-bars)
- [LLM / Agent research layer (v0.2, implemented)](#llm--agent-research-layer-v02-implemented)
...
The timings above come from an actual **fresh clone in a clean virtualenv** (install 28.6s, tests
182.9s, figure check 23.5s). To prove the README figures are generated by code:

```bash
python scripts/reproduce_all.py --verify   # regenerate 8 figures and compare with the committed ones (~24s)
```
...
| **Formulaic alphas** | A clean-room Alpha101 subset, **44 implemented** with the **22 needing industry or market-cap data explicitly skipped and the missing input named**; the evidence chain tightens layer by layer: 41 raw significant cells → 35 (BH-FDR) → 28 (Bonferroni) → 11/16 surviving out of sample → only **3/33** with a net-excess t above 2 after 20 bps costs |
| **LLM / agent layer** | Read-only tools + bounded plan-act-observe loop + full-step trace｜**the same 20 tasks evaluated side by side for a scripted agent and a real model** (grounded-number rate 0.989, abstention 1.000, zero constraint violations)｜`aqlab trace` renders a run as a **single self-contained HTML file** that flags failed calls and ungrounded numbers |
| **Reproducibility** | `scripts/reproduce_all.py --verify` regenerates the README figures and compares them with the committed versions｜`scripts/verify_fresh_clone.py` runs install → tests → figure check inside a **fresh clone** |
| **Documentation** | [Alpha research write-up](docs/ALPHA_RESEARCH.md)｜[Case study](docs/CASE_STUDY.md)｜[Architecture](docs/ARCHITECTURE.md)｜[Roadmap](docs/ROADMAP.md)｜[中文 README](README.md) |

**Stack**: Python 3.10+ ｜ numpy / pandas (the core has no third-party strategy dependencies) ｜ pytest + pytest-cov ｜ GitHub Actions ｜ local LevelDB quote store (C++ engine + Python SDK) ｜ optional: tushare / akshare / matplotlib
...
The three most common failure modes in quantitative research can all be closed off by engineering:

1. **Lookahead** - a signal that reads the same day's close and is then scored on that day's return will
   always look profitable. Every signal here is executed one bar late, and a test asserts it by running a
   signal that peeks at the same day's move and requiring the result to lose money.
2. **Cost illusion** - a high-turnover signal that ignores fees and slippage looks great on paper. Costs are
   modelled into the return here, per trade.
3. **Irreproducibility** - if the data and the randomness are not pinned, nobody can reproduce the result.
   Deterministic synthetic bars with fixed seeds mean `pytest` and the demo produce identical output anywhere.

## More usage

```bash
# three-minute tour: built-in strategies on the same synthetic bars
python -m aqlab.cli demo --seed 25      # rising market
python -m aqlab.cli demo --seed 7       # falling market

# cross-sectional screening (30 synthetic symbols, weighted factor score)
python -m aqlab.cli screen --symbols 30 --days 500 --top 5

# backtest a custom CSV (Chinese column names supported: 日期/开盘/最高/最低/收盘/成交量)
python -m aqlab.cli run --csv data/raw/600519.csv --strategy ma_cross --params fast=10,slow=30

# optional: fetch real daily bars (needs a tushare token or akshare)
...
# agent reliability evaluation (offline, no API key needed)
python -m aqlab.cli eval --mode offline

# run the research agent on a real model (OpenAI-compatible key, e.g. DeepSeek)
# put the key in .env (gitignored) or export it, then check the setup
python -m aqlab.cli doctor
python -m aqlab.cli agent --question "Backtest SYN001 with ma_cross(10,30) and report total return and Sharpe." --trace output/trace.json
```
...
| Document | Contents |
| --- | --- |
| [Alpha research write-up](docs/ALPHA_RESEARCH.md) | The full evidence chain from 44 formulaic alphas to 3 candidates, including multiple-comparison control, walk-forward folds, the cost hurdle and the rejected hypotheses |
| [Case study](docs/CASE_STUDY.md) | The complete research loop: rules → factor research → portfolio → agent evaluation, with methods and limitations |
| [Architecture](docs/ARCHITECTURE.md) | The three-layer split and the responsibility / non-responsibility boundary of all 42 modules |
| [Roadmap](docs/ROADMAP.md) | What each release did and what it measured |
