...
| 维度 | 具体数字 / 事实 |
| --- | --- |
| **测试与质量** | **319 个测试**（32 个测试模块，全部离线、无需网络与 API key）｜**覆盖率 86%**，CI 门禁 `--cov-fail-under=85`｜**ruff 与 mypy 全绿并接入 CI**（lint 规则集在 `pyproject.toml` 里钉死，不用"ALL"再逐个抑制）｜CI 在 Python **3.10 / 3.11 / 3.12** 三版本矩阵上跑 lint + 类型检查 + 测试 + CLI 冒烟 |
...
| **可复现性** | `python scripts/reproduce_all.py --verify` 把 README 里所有图表重新生成并与已提交版本**逐字节比对**（4 张合成图全部一致）；测试还会断言「README 里每张图都有复现步骤」，漏一张即失败 |
...
# 2) 跑测试（319 个用例，全部离线，无需网络/API key）
pytest -q
...
- **校验档**用 sha256 逐字节比对：**已提交的 4 张合成图与现场生成的结果完全一致**（该项目里 matplotlib 输出是确定性的，
  两次运行的 PNG 字节相同），真实数据图在完整档里同样校验。
