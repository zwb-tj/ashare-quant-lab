...
**从零实现、可复现的 A 股研究平台**：执行成本显式、信号无未来函数、统计结论可证伪，
并带一个**可审计的 LLM 研究代理**（tool calling）与一条可测试的 CLI。

![equity curves](docs/assets/equity_curves.png)

> 不是又一个"翻倍策略"仓库。这里把**研究纪律**写进代码：成本要显式、执行要延迟一根 K 线、
> 结论要能被证伪、代理说的每个数字都必须来自工具。**负面结果同样保留**——多条假设被本项目自己的数据否决。
...
- [为什么做这个](#为什么做这个)
- [更多用法](#更多用法)
- [实测输出（可复现，合成行情）](#实测输出可复现合成行情)
- [LLM / Agent 研究层（v0.2，已实现）](#llm--agent-研究层v02已实现)
- [因子有效性研究：IC / IC_IR / 分位收益（v0.17）](#因子有效性研究ic--ic_ir--分位收益v017)
- [把 Agent 的一次运行变成可读的一页（v0.34）](#把-agent-的一次运行变成可读的一页v034)
- [文档](#文档)
- [设计原则](#设计原则)
- [目录结构](#目录结构)
- [路线图](#路线图)
...
pytest -q                      # 432 个用例，全部离线，无需网络与 API key（约 3 分钟）

# 一条零依赖的研究命令（合成数据，约 6 秒）
python -m aqlab.cli factor-ic --symbols 30 --days 500 --out output
```

上面的耗时来自**全新克隆 + 干净虚拟环境**的实测（安装 28.6s、测试 182.9s、图表校验 23.5s）。
想验证"README 里的图确实由代码生成"：

```bash
python scripts/reproduce_all.py --verify   # 重新生成 8 张图并与已提交版本比对（约 24 秒）
```
...
| 维度 | 具体数字 / 事实 |
| --- | --- |
| **工程质量** | **432 个测试**（49 个模块，全部离线）｜**覆盖率 87%**，CI 门禁 `--cov-fail-under=85`｜**ruff + mypy 全绿**并接入 CI｜Python **3.10 / 3.11 / 3.12** 三版本矩阵 |
| **真实数据规模** | 本地 LevelDB 行情库 **5,424 只标的**（日线 2000 年起、分钟线 2025-01 起）｜单次全市场研究 **58,682 笔**交易 |
| **统计严谨性** | 所有结论都给**样本数 + 同期基准超额 + Welch t 值**；基准为全市场等权指数（同入场日、同持有期）｜关键结论做 **24 组口径稳健性检查** |
| **可证伪的结论** | **多条假设被自己的数据否决**并写进文档：选股规则 334 条记录超额 -0.40 ~ -4.23%（24 格中 13 格显著为负）；结构离场把持有期压到 3~5 天且超额转负；组合层五种权重方法全部跑输等权基准 |
| **公式因子研究** | clean-room 实现 **Alpha101 子集 44 个**（依赖行业/市值的 22 个显式跳过并写明原因）；证据链层层收紧：不校正 41 个显著 → BH-FDR 35 → Bonferroni 28 → 样本外存活 11/16 → 扣 20bps 成本后净超额 t>2 仅 **3/33** |
| **LLM / Agent 层** | 只读工具层 + 有界代理循环 + 全步骤 trace｜**同一套 20 任务在离线代理与真实模型上并列评测**（数字落地率 0.989、该弃答时弃答 1.000、约束违规 0）｜`aqlab trace` 输出**单文件自包含 HTML**，失败与未落地数字显式标红 |
| **可复现性** | `scripts/reproduce_all.py --verify` 把 README 里的图重新生成并与已提交版本比对｜`scripts/verify_fresh_clone.py` 在**全新克隆**里跑通安装 → 测试 → 图表校验 |
| **文档** | [因子研究自述](docs/ALPHA_RESEARCH.md)｜[案例研究](docs/CASE_STUDY.md)｜[架构说明](docs/ARCHITECTURE.md)｜[路线图](docs/ROADMAP.md)｜[English README](README.en.md) |
...
flowchart LR
    ...
    E --> F["Surfaces<br/>aqlab CLI (23 commands)<br/>daily pipeline + notify"]
...
# 代理可靠性评测（离线，无需 API key）
python -m aqlab.cli eval --mode offline

# 用真实模型跑研究代理（需 OpenAI 兼容的 API key，如 DeepSeek）
# 把 key 写进 .env（已在 .gitignore 中）或导出环境变量，用 aqlab doctor 检查配置
python -m aqlab.cli doctor
...
## 滚动多折验证：结论在时间上稳不稳（v0.28）

前两节的"样本外存活"都建立在**一次** 50/50 切分上——单次切分的结果可能只是某个时段的运气。
这一节把时间轴切成**连续多折**，每折都做一次"用过去选、用未来验"：

```
折 2: [ 训练(7 截面) ] -> [ 测试(12 截面) 2025-09-22 起 ]
折 3: [  训练(14)   ] -> [ 测试(12) 2025-12-23 起 ]
折 4: [   训练(21)    ] -> [ 测试(12) 2026-03-27 起 ]
折 5: [    训练(28)     ] -> [ 测试(11) 2026-06-26 起 ]
```

**口径**：训练段是**扩张窗口**（从可用截面起点滚到测试段之前，按 `train_fraction=0.6` 取尾部）；
方向**只由训练段 IC 符号决定**；训练段与测试段都要 |t| > 2 才算"存活"。
折**切在因子真正有值的区间上**（因子有 260 根预热期，若按全部交易日等分，前几折的训练段会落在
预热期内被整折跳过——真实数据上 5 折只剩 2 折，这一点是跑出来才发现的，已修）。

**真实全市场结果**（74 个 因子 × 持有期 组合，4 折）：

| 口径 | 数量 |
| --- | ---: |
| 至少被某折的训练段选中 | 23 |
| 被选中 ≥ 2 折 | 16 |
| **被选中 ≥ 2 折且存活率 ≥ 60%（robust）** | **12** |

**稳健因子明细**（按存活折数排序）：

| 因子 | 持有期 | 被选中折数 | 存活折数 | 存活率 | 测试段平均 IC | 测试段平均 t |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| **alpha_013** | 5 | 4 | 3 | 75% | +0.057 | 4.66 |
| **alpha_016** | 5 | 4 | 3 | 75% | +0.061 | 4.64 |
| **alpha_050** | 5 | 4 | 3 | 75% | +0.053 | 4.56 |
| alpha_015 | 5 | 4 | 3 | 75% | +0.043 | 4.16 |
| alpha_003 | 5 | 4 | 3 | 75% | +0.036 | 2.27 |
| alpha_013 | 20 | 4 | 3 | 75% | +0.072 | 2.22 |
| alpha_044 | 5 | 3 | 2 | 67% | +0.048 | 3.79 |
| alpha_044 | 20 | 3 | 2 | 67% | +0.071 | 2.84 |
| alpha_055 / alpha_026 | 5、20 | 3 | 2 | 67% | — | 1.58 ~ 2.80 |

**最重要的发现：折 4 全员失败。** 逐折存活矩阵里，**折 4（测试段 2026-03-27 ~ 2026-06-26）
所有 20 个被选中的组合无一存活**，而折 2、3、5 都有大量组合存活（见下表）：

| 因子/持有期 | 折 2 | 折 3 | 折 4 | 折 5 |
| --- | :---: | :---: | :---: | :---: |
| alpha_013 / 5 | ✅ | ✅ | ❌ | ✅ |
| alpha_013 / 20 | ✅ | ✅ | ❌ | ✅ |
| alpha_016 / 5 | ✅ | ✅ | ❌ | ✅ |
...
**怎么读**：这些因子（全是反转 / 量价背离族）在**大部分时段有效，但存在整段失效的窗口**。
折 4 的测试段恰好是 2026 年二季度——**同一批信号在那段时间集体失效**，这正是"因子有周期性"
的直接证据，也说明**单次 50/50 切分可能恰好切在有利或不利的时段上**（前两节恰好切在有利用的那半年）。

> 结论要收敛到这里：**"样本外存活"必须加上"多数折存活"才算证据**。
> 本项目的链条现在是：`41 个显著` → `35（BH 校正）` → `11（单次样本外）` → `12（多折稳健）` → `3（扣成本后 t>2）`。
> 多折让"存活的因子"从 11 变成 12，看似没缩水，但**逐折矩阵暴露了折 4 的集体失效**——
> 这是单次切分完全看不到的信息。因子的**周期性**（哪段时间有效）比"平均是否有效"更该被写进结论。

> 边界：① 折数只有 4（受数据长度与 260 根预热限制），每折测试段 11~12 个截面，样本仍偏少；
> ② 折边界是按截面等分的，没有按市场状态（牛/熊/震荡）分层——折 4 的失效可能与市场状态有关，未验证；
> ③ 仍未建模成本（上节的成本结论已单独给出：3/33 净超额 t>2）。

## 市场状态依赖性：折 4 的失效能不能被解释（v0.29）

上一节的多折验证发现折 4（测试段 2026-03-27 ~ 2026-06-25）**所有被选中的组合集体失效**。
这一节回答"为什么"：把每个截面日的 IC 按**当天的市场状态**分组统计（状态用等权指数与横截面收益算，
不需要额外数据；0AMV 波段用已有实现并滞后一天）。

**折 4 窗口的市场状态与其它时段明显不同**：
...
折 4 的截面平均 IC **+0.0041、正 IC 占比 49.2%**；其它时段 **+0.0559、正 IC 占比 86.6%**。
也就是说折 4 的失效是**真实的**（不是统计口径问题），而这个窗口恰好是"**动量最低 + 离散度最高**"的一段。

**按市场动量分组**（IC 均值 / t 值）：

| 因子 | 动量低 | 动量中 | 动量高 | 组间极差 |
| --- | ---: | ---: | ---: | ---: |
| alpha_013 | +0.064 / 4.55 | +0.057 / 6.01 | +0.054 / 4.03 | 0.010 |
| alpha_016 | +0.069 / 4.28 | +0.063 / 6.46 | +0.054 / 3.93 | 0.015 |
| alpha_050 | +0.058 / 3.65 | +0.050 / 7.23 | +0.044 / 3.64 | 0.014 |
| alpha_026 | +0.061 / 2.10 | +0.050 / 4.90 | +0.035 / 2.31 | 0.026 |
| alpha_006 | +0.057 / 1.89 | +0.024 / 1.74 | +0.053 / 2.99 | 0.033 |

**按横截面离散度分组**：

| 因子 | 离散低 | 离散中 | 离散高 | 组间极差 |
| --- | ---: | ---: | ---: | ---: |
| alpha_013 | +0.058 / 3.87 | +0.053 / 6.31 | **+0.064 / 4.51** | 0.011 |
| alpha_016 | +0.064 / 3.72 | +0.055 / 6.90 | **+0.070 / 4.49** | 0.015 |
| alpha_026 | +0.065 / 7.26 | +0.019 / 1.40 | +0.073 / 3.15 | 0.053 |
| alpha_006 | +0.072 / 4.71 | +0.003 / 0.18 | +0.073 / 2.93 | 0.071 |

**结论（与我事先的猜测相反，按数据写）**：

1. **"高离散导致失效"这个假设被数据否掉了。** 这些反转 / 量价背离因子在**离散度最高**的一档里
   IC 反而更强（alpha_013：离散高 +0.064 vs 离散中 +0.053；alpha_016：+0.070 vs +0.055）。
   也就是说离散度不是失效的原因——**我原本的直觉是错的**。
2. **折 4 的关键特征是"动量低"**：动量低档的 IC 普遍**高于**动量高档（alpha_026：+0.061 vs +0.035；
   alpha_055：+0.044 vs +0.028）——但折 4 的 IC 却接近零。这说明**"动量低"本身不足以解释失效**，
   还存在这两个状态变量之外的、未识别的因素（或是这段窗口特有的"反转特征整体消失"）。
3. **0AMV 波段最有区分度，但不是普适规律**：10 个因子里**7 个**在**关波段**下的 IC 高于开波段
   （alpha_050：关波段 +0.063 / t=7.94 vs 开波段 +0.038 / t=3.90；alpha_026：+0.068 / 4.82 vs +0.029 / 2.01），
   而 alpha_003 / alpha_006 / alpha_012 **方向相反**（开波段更高）。所以只能说"关波段整体更有利"，
   不能声称这是所有因子共有的规律。
4. 因此对折 4 的解释只能说到这一步：**它是"动量低 + 开波段"叠加的一段，且这段里反转特征整体减弱**；
   但把"动量低"或"开波段"单独拿出来都无法复现这种失效——**不能声称已找到失效的机制**。

> 诚实边界：① 每档只有 29~30 个截面（0AMV 两组各 29/30），分组后样本很薄，t 值只能当方向参考；
> ② 状态档位是**全样本等频分档**，用到了未来信息（真实交易中档位边界只能事后知道），
> 因此这是**解释性分析而非可交易信号**；③ 折 4 只有 12 个截面，把它归因到某个状态本身就证据不足；
> ④ 未检验"状态 × 因子"的交互显著性（只比较了组间均值）。

## 同一套评测，离线代理与真实模型并列（v0.33）

Agent 层的 20 任务可靠性评测可以跑在两种模式上：**离线脚本代理**（确定性，进 CI）与
**真实模型**（`--mode live`，需要 API key）。两者用**完全相同**的任务集与判定口径，
因此可以直接对比——这也是这个项目唯一一处"能与外部模型对标"的证据。

| 指标 | 离线脚本代理 | 真实模型（deepseek-chat） |
| --- | ---: | ---: |
| 任务数 | 20 | 20 |
| 有答案率 | 1.000 | 1.000 |
| 工具调用成功率 | 0.913 | **0.978** |
...
| 数字落地率 | 0.981 | **0.989** |
| 有未落地数字的任务数 | 1 | 2 |
| 该弃答时弃答（陷阱题） | 1.000 | **1.000** |
| 矛盾前提纠正 | 1.000 | 0.667 |
| 约束违规 | 0 | **0** |
| 结论可复现（数字层面） | 1.000 | **1.000** |
| 工具调用稳定（诊断） | 1.000 | 1.000 |
| 逐字一致（参考指标） | 1.000 | 0.000 |

**怎么读**：

1. **真实模型的可靠性接近离线代理**：数字落地率 0.989（离线 0.981）、
   弃答 1.000、约束违规 0、结论可复现 1.000。换句话说，**在"不编数字、该拒就拒"这两件事上，
   真实模型与确定性脚本表现相当**，而这个评测正是为此设计的。
2. **真实模型更强的地方**：工具调用成功率 0.978（离线 0.913）——它会先探查再调用，
   遇到参数缺失时能自我修正。
3. **`逐字一致 = 0.000` 是预期的，也是重要的**：同一问题两次回答**措辞完全不同**，
   但**数字结论 100% 一致**。这说明"可复现性"必须按**结论**衡量，而不是按文本——
   早期版本用逐字比对，把这种正常行为判成了 0（见下）。
4. **`矛盾前提纠正 0.667`（2/3）是真实模型的真实短板**：有一题的措辞让它没有显式拒绝
   未经工具验证的说法。这是**负面结果**，如实保留。

**评测口径被真实数据修正过三次**（这段比数字更有信息量）：

| 发现 | 原判据的问题 | 修正后 |
| --- | --- | --- |
| 模型答"我**不能直接**确认这个数字" | 固定短语 `不能确认` 匹配不上（中间插了"直接"）→ 误判 0 | 主判据改为**行为**：数字有工具依据 或 明确弃答，且不重复错误说法 |
| 模型答`但"肯定赚钱"这个说法我不能背书` | 引用原话以反驳被当成"重复错误说法" | 判定前剥掉引号包裹片段；数字类禁词在明确弃答时豁免 |
| 同一问题两次回答 357 / 312 字符 | **逐字比对**测的是"文本复现"，不是可复现性 | 比较**数字结论集合**（允许表头序号等表述性差异），逐字一致降为参考指标 |

> 反复出现的教训：**一开始我不断往关键词表里加词，本质是拿指标去拟合观测到的输出**——
> 这会把"衡量可靠性"变成"抄答案"。改成行为判据（有没有工具依据 / 有没有明确拒绝 /
> 结论是否复现）之后，判据不再随措辞漂移。

**可证伪性**：两处"未落地数字"经人工核对分别是策略个数（4/3/2）与年化交易日常数（252/50），
属于常识性数字而非编造业绩。主指标**仍然严格计入**它们（不为自己放宽口径），
另给 `suspicious_numbers` 诊断列把两者区分开。

## 把 Agent 的一次运行变成可读的一页（v0.34）

trace 一直是 JSON：完整、可机读，但**演示时看不出代理到底做了什么**。
`aqlab trace` 把它渲染成**单文件 HTML**（样式内联、零外部引用，断网也能打开）：

```bash
aqlab agent --question "用内置的 ma_cross(10,30) 回测 SYN001，告诉我总收益和 Sharpe。" --trace trace.json
aqlab trace --in trace.json --out trace.html
```

页面包含：**问题 → 汇总卡片 → 最终回答 → 逐步时间线**（每一步的文本、调用的工具、参数与返回值）。

仓库里有一份用**真实模型**跑出来的示例：[`docs/assets/agent_trace.html`](docs/assets/agent_trace.html)（对应的原始 trace 是 `docs/assets/agent_trace.json`，可用上面的命令重新渲染）。

**三条设计原则**（都有测试守着）：

1. **自包含**：不引用 CDN、不加载外部字体或脚本 —— 演示时最怕"图打不开、样式错乱"；
2. **不美化**：工具调用**失败**标红、**未落地数字**（答案里在工具输出中找不到的数字）单独提示、
   非 `final` 停止（例如撞上 `max_steps`）标为警示。把失败渲染成成功会让这个视图失去可信度；
3. **确定性**：同一 trace 产出逐字节相同的 HTML，可 diff、可提交。

> 值得一提：「未落地数字」的判定**直接复用评测模块的口径**（`evaluation.grounding_report`），
> 而不是在这里另写一套 —— 视图与评测用同一把尺子，才不会出现"报告说合格、图里却标红"的矛盾。

## 文档

| 文档 | 内容 |
| --- | --- |
| [因子研究自述](docs/ALPHA_RESEARCH.md) | 从 44 个公式因子到 3 个候选的完整证据链，含多重比较校正、多折验证、成本检验与被否决的假设 |
| [案例研究](docs/CASE_STUDY.md) | 完整研究闭环：规则研究 → 因子研究 → 组合检验 → Agent 评测，含方法与局限 |
| [架构说明](docs/ARCHITECTURE.md) | 三层划分与全部 42 个模块的职责/非职责边界 |
| [路线图](docs/ROADMAP.md) | 逐版本记录做了什么、实测结论是什么 |
...
├── tests/                 # 432 个用例：数据、指标、回测（含无未来函数反证）、工具、代理、评测、规则、砖型图、流水线、通知、持仓离场、事件研究、滚动窗口、组合层、参数扫描
...
- ✅ **v0.29** 状态依赖性分析：按市场动量 / 离散度 / 0AMV 波段分组看 IC——关波段下的因子表现一致更好，而折 4 的失效无法用单一状态解释（两个猜测被数据否掉，如实记录）。
- ✅ **v0.28** 多折 walk-forward 验证：4 折扩张窗口滚动，12 个组合稳健；折 4（2026Q2）全员失效，说明因子存在整段失效窗口。
...
- ✅ **v0.13** 面向展示的工程面：CLI 端到端测试把覆盖率从 74% 提到 89%（后续加入因子研究模块后为 87%）、CI 覆盖率门禁 ≥85%、English README、案例研究（含被否决的假设）。
