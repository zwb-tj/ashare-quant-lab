...
| 维度 | 具体数字 / 事实 |
| --- | --- |
| **测试与质量** | **377 个测试**（32 个测试模块，全部离线、无需网络与 API key）｜**覆盖率 87%**，CI 门禁 `--cov-fail-under=85`｜**ruff 与 mypy 全绿并接入 CI**（lint 规则集在 `pyproject.toml` 里钉死，不用"ALL"再逐个抑制）｜CI 在 Python **3.10 / 3.11 / 3.12** 三版本矩阵上跑 lint + 类型检查 + 测试 + CLI 冒烟 |
...
| **状态依赖性分析** | 把 IC 按当日市场状态（等权指数动量 / 横截面离散度 / 0AMV 波段）分组：**关波段下全部 10 个因子的 IC 与 t 值都高于开波段**（α050：+0.063/t=7.94 vs +0.038/t=3.90）；但「高离散导致失效」的猜测被数据否掉（离散最高档 IC 反而更强），折 4 的失效**未被机制解释** |
...
# 2) 跑测试（377 个用例，全部离线，无需网络/API key）
pytest -q
...
3. **0AMV 波段是最有区分度的状态变量**：全部 10 个因子在**关波段**下的 IC 与 t 值都高于开波段
   （alpha_050：关波段 +0.063 / t=7.94 vs 开波段 +0.038 / t=3.90；alpha_026：+0.068 / 4.82 vs +0.029 / 2.01）。
   这是这一节最干净的发现：**这些因子的有效性与"只卖不买"的市场阶段负相关**。
