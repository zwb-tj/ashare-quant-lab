...
| 维度 | 具体数字 / 事实 |
| --- | --- |
| **测试与质量** | **431 个测试**（49 个测试模块，全部离线、无需网络与 API key）｜**覆盖率 87%**，CI 门禁 `--cov-fail-under=85`｜**ruff 与 mypy 全绿并接入 CI**（lint 规则集在 `pyproject.toml` 里钉死，不用"ALL"再逐个抑制）｜CI 在 Python **3.10 / 3.11 / 3.12** 三版本矩阵上跑 lint + 类型检查 + 测试 + CLI 冒烟 |
...
| **工程实践** | 无未来函数（信号统一延迟一根 K 线，并有专门断言它的测试）｜显式手续费/滑点/份额记账/换手约束｜确定性合成数据 + 固定种子，输出可逐字节复现｜CLI **23 个子命令**，其中 **11 个由端到端测试**真跑（含中文列名 CSV、全市场研究、量比确认） |
| **离线 / 真实模型并列评测** | 同一套 20 任务、同一判据跑两种模式：真实模型数字落地率 **0.989**、该弃答时弃答 **1.000**、约束违规 **0**、结论可复现 **1.000**（与离线代理相当），工具调用成功率更高（0.978 vs 0.913）；**矛盾前提纠正 0.667 是真实短板，如实保留**；同一问题两次回答**逐字一致 0.000 但数字结论 100% 一致** |
| **Agent trace 可视化** | `aqlab trace` 把一次运行渲染成**单文件 HTML**（零外部引用、断网可开）：汇总卡片 + 逐步时间线 + 工具参数与返回值；**失败调用、未落地数字、非正常停止都显式标红**，且落地判定**复用评测模块的同一口径** |
...
| **状态依赖性分析** | 把 IC 按当日市场状态（等权指数动量 / 横截面离散度 / 0AMV 波段）分组：**10 个因子里 7 个在关波段下的 IC 高于开波段**（α050：+0.063/t=7.94 vs +0.038/t=3.90；3 个方向相反）；但「高离散导致失效」的猜测被数据否掉（离散最高档 IC 反而更强），折 4 的失效**未被机制解释** |
| **公式因子的多折验证** | 把单次 50/50 切分升级为 **walk-forward 多折**（训练段扩张窗口、方向只由训练段定）：74 个组合中 12 个稳健（≥2 折被选中且存活率 ≥60%）；**逐折矩阵暴露折 4（2026Q2）全员失效**——因子有明显周期性，单次切分可能恰好切在有利时段 |
...
| **文档** | [因子研究自述（从 44 个公式到 3 个候选）](docs/ALPHA_RESEARCH.md) + 中文 README + [English README](README.en.md) + [案例研究（完整研究闭环：量化检验 → 因子研究 → 组合检验 → Agent 评测）](docs/CASE_STUDY.md) + [架构说明](docs/ARCHITECTURE.md) + [路线图](docs/ROADMAP.md) |
...
下面的数字来自**全新克隆 + 干净虚拟环境**的实际运行，不是估计：
...
pytest -q                        # 431 个用例全部通过，约 3 分钟，无需网络与 API key

# 验证"README 里的图确实由代码生成"：重新画一遍并与已提交版本逐字节比对（约 24 秒）
python scripts/reproduce_all.py --verify

# 一条零依赖的研究命令（合成数据，约 6 秒）
python -m aqlab.cli factor-ic --symbols 30 --days 500 --out output

# 可选：把上面这套流程在**全新克隆**里再验一遍（临时目录克隆 -> 安装 -> 测试 -> 图表校验）
python scripts/verify_fresh_clone.py
```

> 上述四步已在全新克隆上逐条跑通（安装 28.6s、测试 182.9s、图表校验 23.5s、研究命令 6.0s）。
> `scripts/reproduce_all.py --verify` 默认只要求**产物能重新生成**；加 `--strict` 才要求逐字节一致
> （图片尺寸受 `bbox_inches="tight"` 的裁剪影响、跨平台字体会不同，所以字节级一致只在同一平台上成立）。
...
# 2) 跑测试（431 个用例，全部离线，无需网络/API key）
pytest -q
...
1. **真实模型的可靠性接近离线代理**：数字落地率 0.989（离线 0.981）、弃答 1.000、
   约束违规 0、结论可复现 1.000。换句话说，**在"不编数字、该拒就拒"这两件事上，
   真实模型与确定性脚本表现相当**，而这个评测正是为此设计的。
2. **真实模型更强的地方**：工具调用成功率 0.978（离线 0.913）——它会先探查再调用，
   遇到参数缺失能自我修正。
3. **`逐字一致 = 0.000` 是预期的，也是重要的**：同一问题两次回答**措辞完全不同**，
   但**数字结论 100% 一致**。这说明"可复现性"必须按**结论**衡量，而不是按文本——
   早期版本用逐字比对，把这种正常行为判成了 0。
4. **`矛盾前提纠正 0.667`（2/3）是真实模型的真实短板**：有一题的措辞让它没有显式拒绝
   未经工具验证的说法。这是**负面结果**，如实保留。
...
| 发现 | 原判据的问题 | 修正后 |
| --- | --- | --- |
| 模型答"我**不能直接**确认这个数字" | 固定短语 `不能确认` 匹配不上（中间插了"直接"）→ 误判为 0 | 主判据改为**行为**：数字有工具依据 或 明确弃答，且不重复错误说法 |
| 模型答`但"肯定赚钱"这个说法我不能背书` | 引用原话以反驳被当成"重复错误说法" | 判定前剥掉引号包裹片段；数字类禁词在明确弃答时豁免 |
| 同一问题两次回答 357 / 312 字符 | **逐字比对**测的是"文本复现"，不是可复现性 | 比较**数字结论集合**（允许表头序号等表述性差异），逐字一致降为参考指标 |
...
├── tests/                 # 431 个用例：数据、指标、回测（含无未来函数反证）、工具、代理、评测、规则、砖型图、流水线、通知、持仓离场、事件研究、滚动窗口、组合层、参数扫描
