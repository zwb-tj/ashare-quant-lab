...
3. **"量比高 + 向上冲"不成立，但"开盘量能逐分钟放大"成立**：量比≥4+向上 超额 -0.70%（比全样本 -0.31% 更差），
   量比越高越差；而量能斜率 Q1 低（量递减）超额 -0.90%（t=-3.68）→ Q4 高（量递增）-0.01%（t=-0.04），**单调改善**。

> 边界：开盘特征只有 2026-06~09 一个窗口（2,597 笔）；超额 t 多在 0.2~1.5，不显著；
> 基准是等权（含小盘股）；未计交易成本与涨跌停不可成交。
