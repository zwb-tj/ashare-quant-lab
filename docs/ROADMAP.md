...
## v0.23 · 组合层接真实数据（✅ 已完成，2026-09）

- `aqlab portfolio --data-dir ... --compare-methods`：同一信号集与约束下对比五种权重方法（等权/逆波动/风险平价/最小方差/均值方差）。
- 真实全市场结论：均值方差集中到 6.6 只、回撤 -43%（估计误差被放大）；最小方差波动最低；年换手 2,280%+，成本成为决定项。
- 退化提示：平均持仓过少时五种方法退化为同一组合，报告明确写出「这种对比没有信息量」。
- 修正：`annual_turnover` / `total_cost` 按百分数显示（此前少乘 100）。

## v0.24 · 组合层参数敏感性（✅ 已完成，2026-09）

- `aqlab portfolio --sensitivity --sensitivity-lookbacks 60,120 --sensitivity-caps 0.1,0.2`：小网格检验方法排名是否稳健。
- 结论：均值方差在 4 个格子全部垫底（-18.18% ~ -31.27%）；收紧上限与拉长窗口只能缓解不能修复；等权对参数不敏感。
- 性能：`project_to_capped_simplex` 内部迭代 200 → 60（实测逐位相同）+ 梯度收敛提前终止 → 最小方差 50.3s→4.9s、均值方差 52.5s→0.7s。

## v0.25 · 公式化 alpha 因子（✅ 已完成，2026-09）

- `src/aqlab/alpha101.py`：clean-room 实现 Alpha101 子集（44 个）+ 完整算子层（时序/截面分向）；依赖行业/市值的 22 个因子在 `SKIPPED` 里写明原因，不用近似值冒充。
...
## v0.27 · 公式因子的成本检验（✅ 已完成，2026-09）

- `src/aqlab/alpha_portfolio.py`：次日开盘入场、方向由样本内 IC 符号决定、按实际换手收费，输出盈亏平衡成本与成本敏感性。
- 真实全市场：33 个组合中毛超额为正 19、净超额为正 17、**净超额 t>2 仅 3**；平均换手 0.90、盈亏平衡成本中位数 23.4bps。
- 修正：方向排序一度写反（`ascending=direction>0` 导致买成最低分位）；并列因子分导致虚假换手（补标的代码兜底排序）。

## v0.28 · 多折 walk-forward 验证（✅ 已完成，2026-09）

- `src/aqlab/walk_forward_folds.py`：4 折扩张窗口；IC 序列只算一次，各折按日期切片（保证各折口径一致）。
- 真实全市场：74 个组合中 12 个稳健；**折 4（测试段 2026-03-27 ~ 2026-06-26）20 个被选中的组合全部失效**。
- 修正：折原本按全部交易日等分，导致训练段落在因子预热期内、5 折只剩 2 折被评估；改为按**因子有值的截面**划分。

## v0.29 · 市场状态依赖性（✅ 已完成，2026-09）

- `src/aqlab/state_dependence.py`：按等权指数动量 / 横截面离散度 / 0AMV 波段分组统计 IC（等频分档）。
- 真实全市场：10 个因子在**关波段**下的 IC 与 t 值**全部高于**开波段；但「高离散导致折 4 失效」的猜测被否掉。
- 待办：状态 × 因子的交互显著性检验；用滚动分档替代全样本分档（当前分档用到了未来信息，属解释性分析）。

## v0.35 · 案例研究收尾（✅ 已完成，2026-09）

- `docs/CASE_STUDY.md` 补上 Agent 层（离线 vs 真实模型并列评测、判据三次修正的经过）与 trace 可视化两节。
- 修正过期数字（32→49 测试模块、86%→87%、5,382→6,566 语句、18→23 子命令），并加守护测试防再次漂移。

## v0.30–v0.32 · 文档与配置（✅ 已完成，2026-09）

- **v0.30 / v0.30.1**：因子研究自述（`docs/ALPHA_RESEARCH.md`）＋把四段分析脚本固化进 `scripts/`（此前只在临时目录，与"可复现"的承诺不符）。
- **v0.31 / v0.31.1 / v0.31.2**：全新克隆验证（`scripts/verify_fresh_clone.py`）、速览声明核对（`scripts/verify_brief_claims.py`）、
  文档数字守卫（测试数/版本号/证据链数字）；修 Python 3.10 兼容事故（无保护的 `import tomllib`）并把 CI 拆成 `verify` / `compat` 两个 job。
- **v0.32**：`env_file.py` 让 `.env` 真正生效（此前 `.env.example` 承诺了却从不读取）＋ `aqlab doctor`（只报来源与长度，不打印密钥）。

## v0.33 · 真实模型评测（✅ 已完成，2026-09）

- `.env` 支持与 `aqlab doctor`：让 `--mode live` 可直接使用（密钥只报长度，不打印内容）。
- 真实模型 20 任务：数字落地 0.989、弃答 1.000、违规 0、结论可复现 1.000；矛盾前提纠正 0.667（短板）。
...
- ✅ 持有期稳健性已完成（v0.19）：多持有期 IC 期限结构（一次遍历，避免重复算）+ 5 日持有期的方案复核。
- ✅ 因子组合方案回测已完成（v0.18）：四种定权方案 + 滚 IC 动态定权 + 成本 + 等权基准（`aqlab factor-backtest`）。
- ✅ 因子研究已完成（v0.17）：IC / IC_IR / 分位收益 + 因子 IC 与分位收益两张图（`aqlab factor-ic`）。
